...
//        let randNonce = Data(challenge)
        ...
        let cmd = NFCISO7816APDU(data: Data.init(challenge))!
        
        send( cmd: cmd, completed: completed )
    }
    
    func doChipAuthentication(mseSetATCApdu: [UInt8], generalAuthCApdu: [UInt8], selectSMCApdu: [UInt8], completed: @escaping (ResponseAPDU?, NFCPassportReaderError?)->() ) {
        self.send(cmd: NFCISO7816APDU(data: Data(mseSetATCApdu))!, completed: {(rapdu, error) in
            if let rapdu = rapdu {
                Log.info("CA OK (MSE): ")
                Log.info(rapdu.data.description)
                Log.info(binToHexRep(rapdu.sw1) + binToHexRep(rapdu.sw2))
                if generalAuthCApdu != nil && generalAuthCApdu.count != 0 {
                    self.send(cmd: NFCISO7816APDU(data: Data(generalAuthCApdu))!, completed: {(rapdu, error) in
                        if let rapdu = rapdu {
                            Log.info("CA OK (GENERAL): ")
                            Log.info(rapdu.data.description)
                            Log.info(binToHexRep(rapdu.sw1) + binToHexRep(rapdu.sw2))
                            completed(rapdu, error)
                        } else {
                            Log.error("CA ERROR (GENERAL): " + error!.errorDescription!)
                        }
                    })
                } else {
                    completed(rapdu, error)
                ...
                Log.error("CA ERROR (MSE): " + error!.errorDescription!)
            }
        })
    ...
    func send( cmd: NFCISO7816APDU, completed: @escaping (ResponseAPDU?, NFCPassportReaderError?)->() ) {
        ...
        tag.sendCommand(apdu: toSend) { [unowned self] (data, sw1, sw2, error) in
            if let error = error {
                ...
                var rep = ResponseAPDU(data: [UInt8](data), sw1: sw1, sw2: sw2)
                Log.debug(binToHexRep([UInt8](data)))
                if let sm = self.secureMessaging {
                    do {
                        rep = try sm.unprotect(rapdu:rep)
                ...
                if rep.sw1 == 0x90 && rep.sw2 == 0x00 {
                    ...
                    completed(rep, tagError)
                }
            }
        }
    }
    
    func sendSelectSMC( cmd: NFCISO7816APDU, completed: @escaping ([UInt8]?, UInt8?, UInt8?, NFCPassportReaderError?)->() ) {
        ...
        if let sm = secureMessaging {
            do {
                ...
                completed(nil, nil, nil,  NFCPassportReaderError.UnableToProtectAPDU )
            }
            Log.debug("TagReader - [SM] \(toSend)" )
        }

        tag.sendCommand(apdu: cmd) { [unowned self] (data, sw1, sw2, error) in
            if let error = error {
                Log.error( "TagReader - Error reading tag - \(error.localizedDescription))" )
                completed([UInt8](data), sw1, sw2, NFCPassportReaderError.ResponseError( error.localizedDescription, sw1, sw2 ) )
            } else {

                var rep = ResponseAPDU(data: [UInt8](data), sw1: sw1, sw2: sw2)
                Log.debug( "TagReader - Received response: " + binToHexRep([UInt8](data)) )
                if let sm = self.secureMessaging {
                    do {
                        rep = try sm.unprotect(rapdu:rep)
                        Log.debug(String(format:"TagReader [SM - unprotected] \(binToHexRep(rep.data, asArray:true)), sw1:0x%02x sw2:0x%02x", rep.sw1, rep.sw2) )
                    } catch {
                        completed([UInt8](data), sw1, sw2, NFCPassportReaderError.UnableToUnprotectAPDU )
                        return
                    }
                } else {
                    Log.debug(String(format:"TagReader [unprotected] \(binToHexRep(rep.data, asArray:true)), sw1:0x%02x sw2:0x%02x", rep.sw1, rep.sw2) )
                ...
                if rep.sw1 == 0x90 && rep.sw2 == 0x00 {
                    completed([UInt8](data), rep.sw1, rep.sw2, nil )
                } else {
                    Log.error( "Error reading tag: sw1 - 0x\(binToHexRep(sw1)), sw2 - 0x\(binToHexRep(sw2))" )
                    let tagError: NFCPassportReaderError
                    if (rep.sw1 == 0x63 && rep.sw2 == 0x00) {
                    ...
                    completed([UInt8](data), rep.sw1, rep.sw2, tagError )
                }
            }
        }
    }
